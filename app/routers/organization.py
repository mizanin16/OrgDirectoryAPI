...
from app.crud.organization import (
    get_organization_crud, get_organizations_crud,
    create_organization_crud, update_organization_crud, delete_organization_crud
)
...
def create_new_organization(org: OrganizationCreate, db: Session = Depends(get_db)):
    try:
        return create_organization_crud(db, org)
...
def update_existing_organization(org_id: int, org_data: OrganizationCreate, db: Session = Depends(get_db)):
    try:
        updated_org = update_organization_crud(db, org_id, org_data)
        if not updated_org:
...
def delete_existing_organization(org_id: int, db: Session = Depends(get_db)):
    deleted_org = delete_organization_crud(db, org_id)
    if not deleted_org:
        raise HTTPException(status_code=404, detail="Organization not found")
...
def read_organizations(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    return get_organizations_crud(db, skip=skip, limit=limit)
